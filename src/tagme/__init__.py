...
class AnnotateResponse(object):
    ...
    def __init__(self, json_content):
        self.annotations = [Annotation(
            ann_json) for ann_json in json_content["annotations"] if "title" in ann_json]
...
class MentionsResponse(object):
    ...
    def __init__(self, json_content):
        self.mentions = [Mention(mention_json)
                         for mention_json in json_content["spots"]]
        self.time = int(json_content["time"])
        self.lang = json_content["lang"]
        self.timestamp = dateutil.parser.parse(json_content["timestamp"])
...
class Relatedness(object):
    ...
    def __init__(self, rel_json):
        self.title1, self.title2 = (wiki_title(t)
                                    for t in rel_json["couple"].split(" "))
...
def _issue_request(api, payload, gcube_token):
    ...
    res_content = res.content.decode(
        "utf-8") if isinstance(res.content, six.binary_type) else res.content
